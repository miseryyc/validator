...
string PeptideFragment::composition ( void ) {
  ...
  for ( int i = 0; i < _iLenPeptide; i++ ) {
    iC += piCompC[_szInputSequence[i]];
    iH += piCompH[_szInputSequence[i]];
    iN += piCompN[_szInputSequence[i]];
    iO += piCompO[_szInputSequence[i]];
    iS += piCompS[_szInputSequence[i]];
  }
